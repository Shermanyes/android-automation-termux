...
class TermuxDeviceController(SystemModule, IDeviceController):
    ...
    def _get_screen_info(self) -> bool:
        ...
        try:
            # 获取屏幕分辨率
            success, output, _ = self._execute_command("wm size", use_root=self._has_root)
            if success:
            ...
            # 获取屏幕密度
            success, output, _ = self._execute_command("wm density", use_root=self._has_root)
            if success:
    ...
    def _check_required_tools(self) -> bool:
        """检查必要的工具是否可用"""
        required_tools = {
            "input": "input tap 0 0",  # 测试点击命令
            "screencap": "screencap --help",  # 测试截图命令
            "am": "am --help",  # 测试Activity Manager
            "wm": "wm --help"  # 测试Window Manager
        }
        ...
        for tool, test_cmd in required_tools.items():
            # 直接测试命令是否可执行，而不是用which查找
            success, output, error = self._execute_command(test_cmd, use_root=self._has_root, timeout=5)

            # 对于某些命令，返回非0也可能是正常的（比如需要参数）
            # 所以我们检查错误信息中是否包含"not found"或"command not found"
            if success or (
                    not success and "not found" not in error.lower() and "command not found" not in error.lower()):
                working_tools.append(tool)
                self.log_debug(f"工具 {tool} 可用")
            else:
                missing_tools.append(tool)
                self.log_debug(f"工具 {tool} 不可用: {error}")

        if missing_tools:
            self.log_warning(f"以下工具可能不可用: {', '.join(missing_tools)}")
            self.log_info(f"可用工具: {', '.join(working_tools)}")

            # 如果基本的input和screencap可用，我们认为可以继续
            if "input" in working_tools and "screencap" in working_tools:
                self.log_info("核心工具(input, screencap)可用，继续初始化")
                return True
            else:
                self.log_error("核心工具不可用，无法继续")
                return False
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self._get_screen_info():
                # 如果无法获取屏幕信息，使用默认值但继续初始化
                self.log_warning("无法获取屏幕信息，使用默认值")
    ...
    def start_app(self, package_name) -> bool:
        ...
        with self._lock:
            try:
                ...
                success, output, _ = self._execute_command(cmd, use_root=self._has_root)
    ...
    def get_device_info(self) -> dict:
        ...
        try:
            # 获取Android版本
            success, output, _ = self._execute_command("getprop ro.build.version.release", use_root=self._has_root)
            if success:
                info['android_version'] = output.strip()

            # 获取设备型号
            success, output, _ = self._execute_command("getprop ro.product.model", use_root=self._has_root)
            if success:
